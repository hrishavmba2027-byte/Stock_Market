...
Output: phased roadmap (P0 → P4), not implementation.
...
│  data/fundamentals.py       ─► quarterly P&L, BS, CF, ratios            │
│  data/news_ingest.py        ─► yfinance news + Reddit + X               │
...
### P1 — New data sources and sentiment
...
**P1.1 — Quarterly fundamentals** (new: `data/fundamentals.py`)
...
- Cache to `Data/fundamentals.parquet`, refresh weekly. Never call yfinance in the inner loop.

**P1.2 — News ingestion** (new: `data/news_ingest.py`)
...
**P1.3 — Reddit ingestion** (new: `data/reddit_ingest.py`)
- **PRAW** (Reddit API, free with personal-use registration).
...
- Match posts/comments to tickers by case-insensitive symbol regex (e.g. `\b(RELIANCE|TCS|INFY)\b`) and aliases (`Reliance Industries`, `Tata Consultancy`). Maintain an alias map at `data/ticker_aliases.json`.
- Store `{ticker, ts, subreddit, title, body, score, num_comments, permalink}`.

**P1.4 — X / Twitter ingestion** (new: `data/x_ingest.py`)
...
**P1.6 — Cross-sectional and regime features** (extend [Feature_Engineering.py](Feature_Engineering.py))
- NIFTY 50 return + 20-day vol; INDIAVIX level + delta; sector index returns.
...
| P1 | `data/` (new) | `fundamentals.py`, `news_ingest.py`, `reddit_ingest.py`, `x_ingest.py` |
| P1 | `features/sentiment.py` (new) | FinBERT scoring + per-source aggregation |
| P1 | `data/ticker_aliases.json` (new) | Alias map for social-text ticker matching |
...
| — | [requirements.txt](requirements.txt) | `lightgbm`, `langchain-anthropic`, `anthropic`, `google-cloud-firestore`, `praw`, `snscrape`, `transformers`, `pyarrow`, `polars`, `mapie`, `pypfopt`, `cvxpy`, `riskfolio-lib` |
...
| P1 | Coverage report: ≥ 70% (ticker, date) rows have non-null `news_sent_mean_7d` and `reddit_sent_mean_7d`. FinBERT scoring of 10k posts < 5 min on CPU. Earnings dates correctly mapped for 50/50 stocks. |
